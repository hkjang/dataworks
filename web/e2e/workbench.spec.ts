...
async function mockPlatform(page: Page) {
  await page.route('**/auth/me', (route) => route.fulfill({ json: { auth_enabled: true, version: 'v0.9.41', user: { id: 'tester', email: 'tester@dataworks.local', name: '테스트 관리자', role: 'super_admin', scopes: ['admin:read', 'admin:write'], default_home: '#/dataworks/home' } } }))
  await page.route('**/auth/sso/status', (route) => route.fulfill({ json: { keycloak_enabled: false, version: 'v0.9.41' } }))
