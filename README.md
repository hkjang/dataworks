...
| `GET/POST` | `/admin/mail/status`, `/admin/mail/deliveries`, `/admin/mail/test` | 사내 SMTP 릴레이 알림(`mail.*` 설정, 기본 꺼짐) 유효 상태(비밀번호 제외)·발송 기록·시험 발송 |
...
`docker-compose.yml`은 이번 오프라인 릴리즈 archive와 동일한 `dataworks:v0.9.54`를 명시적으로 사용합니다. 별도 이미지 버전 환경변수 없이 위 네 필수 환경변수만 설정하면 실행됩니다.
...
이 프로젝트는 **GNU Affero General Public License version 3 (AGPLv3)** 라이선스를 따릅니다. 상세 내용은 [LICENSE](LICENSE) 파일을 참조하세요.
