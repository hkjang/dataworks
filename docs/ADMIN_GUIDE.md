...
| 이벤트 | 받는 사람 | 언제 |
| --- | --- | --- |
| `approval.requested` | 활성 관리자(`admin`·`super_admin`) | 거버넌스 정책이 요청을 승인 대기로 세웠을 때. 24시간 안에 결정하지 않으면 요청은 만료됩니다 |
| `approval.decided` | 요청한 계정 | 관리자가 승인·거절했을 때. 요청자가 다시 시도해도 되는지 알려 줍니다 |
...
- 한 번의 일이 여러 알림을 만들면 묶습니다. 만료 요약은 하루 한 통이고, 재시작하거나 파드가 여럿이어도 발송 기록을 보고 같은 날 두 번 보내지 않습니다.
...
메일은 **배경에서** 보냅니다. 릴레이가 느리거나 죽어 있어도 승인·변경 세트·알림 규칙 요청은 평소대로 200 으로 끝나고, 실패는 발송 기록에 `failed` 로 남습니다(한 번 재시도한 뒤). 시험 발송만 결과를 기다립니다.
