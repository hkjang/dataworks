...
이 문서는 폐쇄망 설치, 최초 관리자 로그인, AI·MCP, Keycloak SSO, 키 정책과 데이터 상품 운영 절차를 실제 v0.9.54 구현 기준으로 설명합니다.

> 이 문서의 모든 화면은 v0.9.54을 실제로 띄워 가상 데이터로 촬영했습니다. 실제 운영 키와 고객 정보가 아닙니다.
...
운영자가 컨테이너에 전달해야 하는 설정은 이 네 항목입니다. AI 공급자, Keycloak과 런타임 정책은 기동 후 관리자 화면에서 저장합니다. 나머지 환경 변수 전체는 [4.7 환경 변수 전수 표](#47-환경-변수-전수-표)에 있습니다.
...
```bash
gzip -t dataworks-v0.9.54.tar.gz
gunzip -c dataworks-v0.9.54.tar.gz | docker load
docker image inspect dataworks:v0.9.54
```
...
3. 로그인 화면 또는 프로필 메뉴에서 `v0.9.54`를 확인합니다.
...
- **관리자 설정** `/dataworks/settings`: AI 공급자, AI·MCP 정책, Keycloak, 방문 추적, 메일 알림과 전체 런타임 설정
...
### 4.6 메일 알림 (사내 SMTP 릴레이)

`관리자 설정 → 메일 알림`에서 사람이 실제로 기다리는 일을 사내 SMTP 릴레이로 알립니다. 기본값은 **꺼짐**이며, 새로 설치한 곳은 켜기 전까지 아무것도 달라지지 않습니다. 설정은 다른 런타임 설정과 같이 DB에 저장되고 화면에서 바꿉니다(환경변수 없음, 재배포 불필요). 폐쇄망에서는 릴레이 주소로 사내 메일 서비스 `postra` 를 가리키면 알림이 밖으로 나가지 않습니다.
...
| `mail.notify_expiring` | 계약·접근권 만료 임박 일일 요약(→ 관리자, 하루 한 통) | `true` |
...
"이 메일이 오지 않으면 누군가 손해를 보거나 화면을 계속 새로고침한다"는 것만 고릅니다. 단순히 무언가 바뀌었다는 알림은 보내지 않습니다.

| 이벤트 | 받는 사람 | 언제 |
| --- | --- | --- |
| `approval.requested` | 활성 관리자(`admin`·`super_admin`) | 거버넌스 정책이 요청을 승인 대기로 세웠을 때. 같은 요청자(계정 또는 API 키)는 10분에 한 통만 보내고, 그 사이의 추가 요청은 메일 없이 대기열에만 쌓입니다. 24시간 안에 결정하지 않으면 요청은 만료됩니다 |
| `approval.decided` | 요청한 계정 | 관리자가 승인·거절했을 때. 승인이면 `X-Governance-Approval-ID` 헤더에 승인 id 를 넣어 다시 보내라고 안내합니다(헤더 없이 보내면 새 승인 요청으로 다시 대기합니다) |
| `change_set.submitted` | 신청자를 뺀 활성 관리자 | 설정 변경 세트를 검토 요청(`submit`)했을 때 |
| `alert.fired` | 활성 관리자 | 알림 규칙이 임계치에 도달했을 때(규칙의 윈도우가 재발송을 막습니다). 웹훅과 별개로 나갑니다 |
| `access.expiring` | 활성 관리자 | 30일 안에 만료되는 활성 계약·접근권이 있으면 매일(UTC 자정 이후 첫 평가, 09:00 KST) **한 통으로 묶어서**. 없으면 보내지 않습니다 |

시끄러움을 막는 규칙:

- **자기가 한 일은 자기에게 보내지 않습니다.** 변경 세트를 낸 관리자는 그 검토 요청 메일을 받지 않습니다.
- 한 번의 일이 여러 알림을 만들면 묶습니다. 만료 요약은 하루 한 통이고, 재시작하거나 파드가 여럿이어도 발송 기록을 보고 같은 날 두 번 보내지 않습니다. 승인 요청은 요청자마다 10분에 한 통이라, 헤더 없이 같은 요청을 되풀이하는 클라이언트나 에이전트 루프가 관리자 전원에게 요청당 한 통씩 쏟아 내지 못합니다.
- 관리자는 `mail.notify_*` 로 종류별로 끌 수 있습니다. 한 스위치를 꺼도 다른 종류는 그대로 나갑니다.
- 수신자는 계정 표(`users`)의 활성 계정에서만 찾습니다. 메일 기능은 자기 사용자 명부를 갖지 않으며, 계정을 비활성화하면 그 순간부터 메일도 멎습니다.

#### 요청을 막지 않는다

메일은 **배경에서** 보냅니다. 릴레이가 느리거나 죽어 있어도 승인·변경 세트·알림 규칙 요청은 평소대로 200 으로 끝나고, 실패는 발송 기록에 `failed` 로 남습니다(한 번 재시도한 뒤). 동시에 보내는 메일은 파드당 64통까지이고, 그 이상은 시작하지 않고 `failed`(`too many mail deliveries in flight`) 로 기록해 죽은 릴레이가 발송 작업을 무한정 쌓지 못하게 합니다. 시험 발송만 결과를 기다립니다.

#### 발송 기록과 비밀번호

- `관리자 설정 → 메일 알림 → 발송 기록`은 시도마다 남깁니다 — 언제, 어떤 이벤트로, 누구에게, 제목이 무엇이었고, 되었는지 안 되었는지. "안 왔다"는 문의에 이 표로 답합니다. **본문은 저장하지 않습니다.**
- `mail.password` 는 저장 후 설정 API 가 돌려주지 않습니다. 화면에는 `설정됨`만 보이고, 바꿀 때만 새 값을 입력하며, 로그에도 남지 않습니다(다른 비밀 설정과 같이 암호화 저장).

```http
GET  /admin/mail/status        # 유효 설정(비밀번호 제외), 미완료 사유, 이벤트 스위치
GET  /admin/mail/deliveries    # 발송 기록 (?status=sent|failed|queued, ?event=…, ?limit=)
POST /admin/mail/test          # {"recipient":"me@example.com"} → 저장된 설정으로 실제 한 통 발송
```
