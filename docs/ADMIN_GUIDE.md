...
이 문서는 폐쇄망 설치, 최초 관리자 로그인, AI·MCP, Keycloak SSO, 키 정책과 데이터 상품 운영 절차를 실제 v0.9.58 구현 기준으로 설명합니다.

> 이 문서의 모든 화면은 v0.9.58을 실제로 띄워 가상 데이터로 촬영했습니다. 실제 운영 키와 고객 정보가 아닙니다.
...
```bash
gzip -t dataworks-v0.9.58.tar.gz
gunzip -c dataworks-v0.9.58.tar.gz | docker load
docker image inspect dataworks:v0.9.58
```
...
3. 로그인 화면 또는 프로필 메뉴에서 `v0.9.58`를 확인합니다.
