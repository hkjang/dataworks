...
이 문서는 폐쇄망 설치, 최초 관리자 로그인, AI·MCP, Keycloak SSO, 키 정책과 데이터 상품 운영 절차를 실제 v0.9.61 구현 기준으로 설명합니다.
...
```bash
gzip -t dataworks-v0.9.61.tar.gz
gunzip -c dataworks-v0.9.61.tar.gz | docker load
docker image inspect dataworks:v0.9.61
```
...
3. 로그인 화면 또는 프로필 메뉴에서 `v0.9.61`를 확인합니다.
