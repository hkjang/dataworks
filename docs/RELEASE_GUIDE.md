...
저장소의 `docker-compose.yml`은 현재 오프라인 릴리즈 이미지 `dataworks:v0.9.47`에 고정되어 있습니다. `dataworks-v0.9.47.tar.gz`를 적재한 뒤 `.env`에는 다음 네 필수 항목만 설정합니다. `GATEWAY_VERSION` 같은 별도 이미지 태그 환경변수는 사용하지 않습니다.
