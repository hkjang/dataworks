...
var apiEndpoints = []apiEndpoint{
	...
	{"/admin/tracking/status", []string{"get"}, "settings", "Visitor tracking: effective configuration, snippet and policy sources", false},
	{"/admin/tracking/violations", []string{"get", "delete"}, "settings", "Visitor tracking: origins the page policy blocked (list / forget)", false},
	{"/admin/tracking/violations/allow", []string{"post"}, "settings", "Visitor tracking: add a blocked origin to tracking.allowed_hosts", false},
	{"/admin/mail/status", []string{"get"}, "settings", "Mail notifications: effective relay configuration (password never returned)", false},
	{"/admin/mail/deliveries", []string{"get"}, "settings", "Mail notifications: delivery ledger (every attempt, sent or failed)", false},
	{"/admin/mail/test", []string{"post"}, "settings", "Mail notifications: send one test message with the saved settings", false},
	{"/tracking/csp-report", []string{"post"}, "ops", "Browser CSP violation reports while visitor tracking is on", true},
	{"/momento/{path}", []string{"get", "post"}, "ops", "Same-origin proxy to the Momento collector (only while tracking is on)", true},
	...
	{"/admin/dataworks/metadata/search", []string{"get"}, "metadata", "Search datasets, metrics, products, APIs, flows, agents, tools, prompts, and reports", false},
	{"/admin/dataworks/metadata/entities", []string{"get", "post"}, "metadata", "List or register unified metadata entities", false},
	{"/admin/dataworks/metadata/edges", []string{"post"}, "metadata", "Register a typed relationship between metadata entities", false},
	{"/admin/dataworks/metadata/{urn}/lineage", []string{"get"}, "metadata", "Traverse upstream and downstream metadata lineage", false},
	{"/admin/dataworks/metadata/{urn}/impact", []string{"get"}, "metadata", "Analyze affected products, APIs, flows, and agents", false},
	{"/admin/dataworks/semantic/metrics", []string{"get", "post"}, "semantic-layer", "List or register governed business metrics", false},
	{"/admin/dataworks/semantic/glossary", []string{"get", "post"}, "semantic-layer", "List or register shared business glossary terms", false},
	{"/admin/dataworks/semantic/assertions", []string{"get", "post"}, "semantic-layer", "List or register data contract assertions", false},
	{"/admin/dataworks/flows", []string{"get", "post"}, "flow-studio", "List or save data and agent flow definitions", false},
	{"/admin/dataworks/flows/{id}", []string{"get"}, "flow-studio", "Flow canvas, nodes, edges, and recent runs", false},
	{"/admin/dataworks/flows/{id}/run", []string{"post"}, "flow-studio", "Execute a flow in the governed orchestration sandbox", false},
	{"/admin/dataworks/flows/{id}/promote", []string{"post"}, "flow-studio", "Promote a flow through draft, test, approved, and production", false},
	{"/admin/dataworks/flows/{id}/runs", []string{"get"}, "flow-studio", "List flow execution history", false},
	{"/admin/dataworks/agents", []string{"get", "post"}, "agent-control", "List or register governed agents", false},
	{"/admin/dataworks/agents/{id}/run", []string{"post"}, "agent-control", "Run an agent with tool, cost, policy, and approval controls", false},
	{"/admin/dataworks/agents/{id}/trace", []string{"get"}, "agent-control", "Inspect agent sessions and execution trace", false},
	{"/admin/dataworks/tools", []string{"get", "post"}, "tool-governance", "List or register MCP, API, SQL, RAG, and product tools", false},
	{"/admin/dataworks/tools/{id}/test", []string{"post"}, "tool-governance", "Validate a tool contract in a non-networked sandbox", false},
	{"/admin/dataworks/tools/{id}/permissions", []string{"get", "post"}, "tool-governance", "List or assign least-privilege tool permissions", false},
	{"/admin/dataworks/policies/simulate", []string{"get", "post"}, "trust", "Simulate financial-data, external-sharing, AI-use, and tool policies", false},
	{"/admin/dataworks/synthetic/generate", []string{"post"}, "synthetic-data", "Generate non-person synthetic or masked API and PoC samples", false},
	{"/admin/dataworks/synthetic/datasets", []string{"get"}, "synthetic-data", "List synthetic dataset generation history", false},
	{"/admin/dataworks/products/{key}/unit-economics", []string{"get"}, "monetization", "Calculate product revenue, cost, margin, and break-even calls", false},
	{"/admin/dataworks/marketplace/items", []string{"get"}, "marketplace", "Search approved products, agents, flows, tools, prompts, and reports", false},
	{"/admin/dataworks/marketplace/subscribe", []string{"post"}, "marketplace", "Request purpose-bound access to a marketplace item", false},
	{"/admin/dataworks/agentops", []string{"get"}, "agent-control", "Agent success, failure, latency, cost, policy-block, and approval metrics", false},
	{"/admin/dataworks/executive/simulate", []string{"post"}, "monetization", "Compare revenue, margin, risk, and PoC outcome scenarios", false},
	...
	{"/admin/k8s/home", []string{"get"}, "k8s", "Cross-cluster operations home (risk / failure candidates / recent changes)", false},
	{"/admin/k8s/connectivity", []string{"get"}, "k8s", "Cluster API connectivity roll-up", false},
	{"/admin/k8s/security", []string{"get"}, "k8s", "Cluster security findings", false},
	{"/admin/k8s/manifest", []string{"get"}, "k8s", "Render a resource manifest as YAML with secrets masked", false},
	{"/admin/k8s/images", []string{"get"}, "k8s", "Image → workload usage and supply-chain risk flags", false},
	{"/admin/k8s/freshness", []string{"get"}, "k8s", "Inventory freshness scores per cluster/namespace/kind", false},
	{"/admin/k8s/discovery", []string{"get"}, "k8s", "Cached API discovery registry for a cluster", false},
	{"/admin/k8s/discovery/compare", []string{"get"}, "k8s", "Diff two clusters' API catalogs", false},
	{"/admin/k8s/revisions", []string{"get"}, "k8s", "Recorded spec revisions for a resource or cluster", false},
	{"/admin/k8s/diff", []string{"get"}, "k8s", "Field-level diff between two revisions of a resource", false},
	{"/admin/k8s/timeline", []string{"get"}, "k8s", "Merged revision / event / action timeline", false},
	{"/admin/k8s/service-impact", []string{"get"}, "k8s", "Service-impact home for a cluster", false},
	{"/admin/k8s/resource-advisor", []string{"get"}, "k8s", "Resource request/limit recommendations", false},
	{"/admin/k8s/remediation/advice", []string{"get"}, "k8s", "Prioritized remediation advice for current RCA findings", false},
	{"/admin/k8s/capacity", []string{"get"}, "k8s", "Cluster capacity headroom", false},
	{"/admin/k8s/capacity/simulate", []string{"get"}, "k8s", "Simulate a scale change against capacity", false},
	{"/admin/k8s/slo", []string{"get"}, "k8s", "Per-namespace SLO / error budget from incident history", false},
	{"/admin/k8s/reports", []string{"get"}, "k8s", "Deterministic daily/weekly/monthly operations report", false},
	{"/admin/k8s/report-schedules", []string{"get", "post"}, "k8s", "List/create report delivery schedules", false},
	{"/admin/k8s/report-schedules/{id}", []string{"delete"}, "k8s", "Delete a report delivery schedule", false},
	{"/admin/k8s/report-schedules/{id}/send", []string{"post"}, "k8s", "Send a scheduled report immediately", false},
	{"/admin/k8s/groups", []string{"get", "post"}, "k8s", "List cluster groups with roll-up / create-update a group", false},
	{"/admin/k8s/groups/{id}", []string{"delete"}, "k8s", "Delete a cluster group", false},
	{"/admin/k8s/ownership", []string{"get", "post"}, "k8s", "List/set namespace ownership", false},
	{"/admin/k8s/incidents", []string{"get", "post"}, "k8s", "List incidents / scan RCA and open incidents", false},
	{"/admin/k8s/incidents/{id}", []string{"get"}, "k8s", "Get one incident", false},
	{"/admin/k8s/incidents/{id}/resolve", []string{"post"}, "k8s", "Resolve an open incident", false},
	{"/admin/k8s/pod-watches", []string{"get", "post"}, "k8s", "List Pod watches with health roll-up / register a watch", false},
	{"/admin/k8s/pod-watches/{id}", []string{"delete"}, "k8s", "Delete a Pod watch", false},
	{"/admin/k8s/policies", []string{"get", "post"}, "k8s", "List/create policy-pack entries", false},
	{"/admin/k8s/policies/{id}", []string{"delete"}, "k8s", "Delete a policy-pack entry", false},
	{"/admin/k8s/policies/compliance", []string{"get"}, "k8s", "Run the enabled policies across the inventory", false},
	{"/admin/k8s/policies/export", []string{"get"}, "k8s", "Export enabled policies as Kyverno or Rego", false},
	{"/admin/k8s/policies/import", []string{"post"}, "k8s", "Import Kyverno/Rego rules as Clustara policies (dry-run capable)", false},
	{"/admin/k8s/policies/simulate", []string{"post"}, "k8s", "Evaluate a manifest against the enabled policies before apply", false},
	{"/admin/k8s/rbac", []string{"get"}, "k8s", "Operational RBAC reference: capability catalog and role matrix", false},
	{"/admin/k8s/rbac/check", []string{"get"}, "k8s", "Preflight a role+capability pair against the RBAC matrix", false},
	{"/admin/k8s/rbac-diff", []string{"get"}, "k8s", "Role/ClusterRole permission expansions between revisions", false},
	{"/admin/k8s/registries/pull-secret", []string{"post"}, "k8s", "Generate an imagePullSecret manifest (credential never persisted)", false},
	{"/admin/k8s/stacks", []string{"get", "post"}, "k8s", "List or create/update Application Stacks", false},
	{"/admin/k8s/stacks/validate", []string{"post"}, "k8s", "Dry-run a multi-document manifest against policies", false},
	{"/admin/k8s/stacks/{id}", []string{"get", "delete"}, "k8s", "Get/delete an Application Stack", false},
	{"/admin/k8s/stacks/{id}/drift", []string{"get"}, "k8s", "Drift between a stack's manifest and the live cluster", false},
	{"/admin/k8s/stacks/{id}/{command}", []string{"post"}, "k8s", "Apply / promote / rollback a stack, or read its history", false},
	{"/admin/k8s/config-changes", []string{"get", "post"}, "k8s", "List/create ConfigMap+Secret change-control requests", false},
	{"/admin/k8s/config-changes/{id}", []string{"get"}, "k8s", "Get one config change request", false},
	{"/admin/k8s/config-changes/{id}/{command}", []string{"post"}, "k8s", "Approve / reject / apply / verify a config change request", false},
	{"/admin/k8s/config-impact", []string{"get"}, "k8s", "Blast radius of a ConfigMap/Secret change", false},
	{"/admin/k8s/cost", []string{"get"}, "cost", "Estimated monthly cluster cost by namespace/team/group", false},
	{"/admin/k8s/cost/config", []string{"get", "post"}, "cost", "Read/set cluster cost unit prices", false},
	{"/admin/k8s/cost/recommendations", []string{"get"}, "cost", "Right-sizing recommendations with monthly saving", false},
	{"/admin/k8s/cost/snapshot", []string{"post"}, "cost", "Record today's per-namespace cost snapshot", false},
	{"/admin/k8s/cost/trend", []string{"get"}, "cost", "Day-over-day cost change per namespace", false},
	{"/admin/k8s/collection-cost", []string{"get", "post"}, "cost", "Collection Cost Guard: storage footprint and budget check", false},
	{"/admin/k8s/collect-config", []string{"get", "post"}, "k8s", "Adaptive collection scheduler config", false},
	{"/admin/k8s/collect-bursts", []string{"get", "post"}, "k8s", "List or open change-aware collection burst windows", false},
	{"/admin/k8s/collect-slo", []string{"get"}, "k8s", "Collector SLO summary and recent classified failures", false},
	{"/admin/k8s/latency/config", []string{"get", "post"}, "k8s", "Read/set the latency PromQL and label mapping", false},
	{"/admin/k8s/latency/collect", []string{"post"}, "k8s", "Pull per-workload latency from Prometheus into metric samples", false},
	{"/admin/k8s/notify/config", []string{"get", "post"}, "k8s", "K8s notification quiet hours and team→channel routing", false},
	{"/admin/k8s/notify/scan", []string{"post"}, "k8s", "Scan RCA/security findings and post routed notifications", false},
	{"/admin/k8s/dw/bootstrap", []string{"post"}, "dw", "Create the K8s fact tables in ClickHouse", false},
	{"/admin/k8s/dw/sink", []string{"post"}, "dw", "Push current K8s facts to ClickHouse", false},
	{"/admin/k8s/dw/report", []string{"get"}, "dw", "Daily aggregation over the K8s fact tables", false},
	{"/admin/k8s/ai/ask", []string{"post"}, "k8s", "Answer an operations question grounded in cluster evidence", false},
	{"/admin/k8s/ai/report", []string{"post"}, "k8s", "Generate an operations summary from cross-cluster aggregates", false},

	// ---- admin: Ops Agent (floating agent) ----
	{"/admin/agent/sessions", []string{"post"}, "agent-control", "Create a floating Ops Agent session with page context", false},
	{"/admin/agent/sessions/{id}", []string{"get"}, "agent-control", "Get a session with its message history", false},
	{"/admin/agent/messages", []string{"post"}, "agent-control", "Ask a question in a session (read-only, evidence grounded)", false},
	{"/admin/agent/suggestions", []string{"get"}, "agent-control", "Context-aware suggested prompts and resolved intent", false},
	{"/admin/agent/evaluations", []string{"get"}, "agent-control", "Recent answer evaluations (?stats=true for the roll-up)", false},
	{"/admin/agent/evaluations/feedback", []string{"post"}, "agent-control", "Record operator thumbs feedback on an answer", false},
	{"/admin/agent/regression", []string{"get", "post"}, "agent-control", "Run the deterministic regression suite / save a baseline", false},
	{"/admin/agent/action-cards", []string{"get", "post"}, "agent-control", "List or propose action cards (the agent never executes)", false},
	{"/admin/agent/action-cards/{id}/status", []string{"post"}, "agent-control", "Advance an action card through its lifecycle", false},
	{"/admin/agent/action-outcomes", []string{"get"}, "agent-control", "Adoption, success, rollback and recurrence of action cards", false},

	// ---- admin: MCP tool scopes ----
	{"/admin/mcp/tool-scopes", []string{"get", "post"}, "mcp", "List/upsert MCP tool scope policies", false},
	{"/admin/mcp/tool-scopes/{server}/{tool}", []string{"delete"}, "mcp", "Delete one MCP tool scope policy", false},

	// ---- admin: Data Works (action center / marketplace / policy) ----
	{"/admin/dataworks/action-center", []string{"get"}, "dataworks", "Prioritized operational follow-ups", false},
	{"/admin/dataworks/assets/readiness", []string{"get", "post"}, "dataworks", "Asset readiness scoring", false},
	{"/admin/dataworks/customer-segments", []string{"get", "post"}, "dataworks", "Customer segments for data product targeting", false},
	{"/admin/dataworks/funnel", []string{"get"}, "dataworks", "Factory-to-revenue lifecycle counts", false},
	{"/admin/dataworks/prompt-templates", []string{"get", "post"}, "factory", "Reusable factory prompt templates", false},
	{"/admin/dataworks/policy/rules", []string{"get", "post"}, "governance", "Policy-as-code rules for data products", false},
	{"/admin/dataworks/marketplace/products", []string{"get"}, "marketplace", "Internal data product marketplace listings", false},
	{"/admin/dataworks/marketplace/bookmarks", []string{"get", "post"}, "marketplace", "List/create marketplace bookmarks", false},
	{"/admin/dataworks/marketplace/subscriptions", []string{"get", "post"}, "marketplace", "List/create marketplace subscriptions", false},
	{"/admin/dataworks/proposal-experiments", []string{"get", "post"}, "dataworks", "List/create proposal copy experiments", false},
	{"/admin/dataworks/proposal-experiments/{id}/feedback", []string{"post"}, "dataworks", "Record positive/negative feedback on a proposal experiment", false},

	// ---- data product runtime / chatops ----
	{"/v1/data-products/{key}/query", []string{"post"}, "data-products", "Query a published data product through its runtime endpoint", false},
	{"/integrations/mattermost/command", []string{"post"}, "notifications", "Mattermost slash-command ingest (form-encoded)", true},
