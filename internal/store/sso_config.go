...
type SSOProviderConfig struct {
	...
	AutoLogin       bool              `json:"auto_login"` // silent prompt=none sign-in; default off
...
func (s *SQLStore) GetSSOProviderConfig(ctx context.Context, provider string) (SSOProviderConfig, bool, error) {
	...
	err := s.db.QueryRowContext(ctx, s.bind(`SELECT provider, enabled, issuer_url, client_id, client_secret_enc,
		redirect_uri, scopes, default_role, role_claim, group_claim, allow_local_login, COALESCE(role_map,''),
		COALESCE(auto_login,0), updated_at, updated_by
		...
		Scan(&c.Provider, &enabled, &c.IssuerURL, &c.ClientID, &c.ClientSecretEnc, &c.RedirectURI,
			&scopes, &c.DefaultRole, &c.RoleClaim, &c.GroupClaim, &allow, &roleMapJ, &autoLogin, &c.UpdatedAt, &c.UpdatedBy)
...
func (s *SQLStore) SaveSSOProviderConfig(ctx context.Context, c SSOProviderConfig) error {
	...
	_, err := s.db.ExecContext(ctx, s.bind(`INSERT INTO sso_provider_config
		(provider, enabled, issuer_url, client_id, client_secret_enc, redirect_uri, scopes,
		 default_role, role_claim, group_claim, allow_local_login, role_map, auto_login, updated_at, updated_by)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(provider) DO UPDATE SET
			...
			role_map = excluded.role_map, auto_login = excluded.auto_login,
			updated_at = excluded.updated_at, updated_by = excluded.updated_by`),
		c.Provider, enabled, c.IssuerURL, c.ClientID, c.ClientSecretEnc, c.RedirectURI,
		strings.Join(c.Scopes, " "), c.DefaultRole, c.RoleClaim, c.GroupClaim, allow, roleMapJ, autoLogin, c.UpdatedAt, c.UpdatedBy)
	return err
}
